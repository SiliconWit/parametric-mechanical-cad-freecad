...
**CrossLink** = BaseLength × ScalingRatio
...
<Tabs>
  ...
  <TabItem label="Build Sequence">
    ...
    ```
    Step 1: Create Spreadsheet (parameters and formulas)
    Step 2: Create Master Sketch (kinematic layout)
    Step 3: Create Link OA
    Step 4: Create Link OB
    Step 5: Create Link AB
    Step 6: Create Input Arm (OI)
    Step 7: Create Output Arm (OP)
    Step 8: Create Base
    Step 9: Assemble with constraints
    Step 10: Test parametric control
    ```
...
<Tabs>
  ...
  <TabItem label="Calculated Values">
    ...
    **Now the powerful part - expressions!**
    ...
    | **B4** | =B3*B2 | BaseLength × k | Auto-calculated! |
    | **C4** | mm | - | Unit |
    | **D4** | OI = BaseLength × k | Note |
    | **A5** | OutputLength | - | Parameter name |
    | **B5** | =B3*B2*B2 | BaseLength × k² | Auto-calculated! |
    | **C5** | mm | - | Unit |
    | **D5** | OP = BaseLength × k² | Note |
    | **A6** | CrossLink | - | Parameter name |
    | **B6** | =B3*B2 | BaseLength × k | Auto-calculated! |
    | **C6** | mm | - | Unit |
    | **D6** | AB = IP = InputLength | Note |
...
4. **Add dimensional callout**
   ...
   Link Lengths:
   OA = OB = 100 mm (BaseLength)
   OI = 200 mm (k × BaseLength)
   OP = 400 mm (k² × BaseLength)
   AB = 200 mm (k × BaseLength)
   ```
